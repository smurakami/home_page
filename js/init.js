...
(function() {
  ...
  Main = (function() {
    ...
    Main.prototype.initMainCanvas = function() {
      ...
      img_srcs = (function() {
        var j, ref, results;
        results = [];
        for (i = j = 0, ref = ANIMATION_IMG_NUM; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
          results.push("img/chara" + i + ".png");
        }
        return results;
      })();
    ...
    Main.prototype.preloadImgs = function() {
      ...
      results = [];
      for (i = j = 0, ref = ANIMATION_IMG_NUM; 0 <= ref ? j < ref : j > ref; i = 0 <= ref ? ++j : --j) {
        results.push($('<img>').attr('src', "img/chara" + i + ".png"));
      }
      return results;
    };
  ...
  ImgAnimationCanvas = (function() {
    ...
    function ImgAnimationCanvas(canvasId, img_srcs) {
      ...
      this.imgs = [];
      for (j = 0, len = img_srcs.length; j < len; j++) {
        src = img_srcs[j];
        img = new Image();
        img.src = src;
        this.imgs.push(img);
